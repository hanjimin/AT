...
public class DateDetailActivity extends ATCustomFragmentActivity {
    ...
    private int selectedCharIndex = 0;  // 캐릭터 선택완료가 되었을 때 캐릭터 index
    private int tempBuyIndex; // 구매완료 이전의 임시 캐릭터 index
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(recipe_type == null){
            recipe_type = "no_recipe";
        }

        if(recipe_type.equals("exam")){
            title.setText(getResources().getString(R.string.default_exam));
            titleCompleted = true;
        } else if(recipe_type.equals("love")){
            title.setText(" ♥ ");
            titleCompleted = true;
        }
        ...
        if(recipe_type.equals("exam") || recipe_type.equals("love")){
